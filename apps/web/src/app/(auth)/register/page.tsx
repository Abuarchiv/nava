...
import { Mail, User, ArrowRight, Loader2, CheckCircle, Home, Users } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'

type WgMode = 'create' | 'join'
...
export default function RegisterPage() {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email: email.trim(),
      options: {
        emailRedirectTo: `${origin}/auth/callback?next=${wgMode === 'create' ? '/onboarding' : '/join'}`,
        data: {
  ...
  async function handleGoogle() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${origin}/auth/callback?next=${wgMode === 'create' ? '/onboarding' : '/join'}`,
      },
    })
          ...
          {sent ? (
            <div className="text-center py-4">
              ...
              <p className="text-[#9a8f7c] text-sm leading-relaxed mb-8">
                ...
                <span className="text-[#f7f3ec] font-medium">{email}</span>. Klick rein.
              </p>
              <button
                type="button"
            ...
            <>
              ...
              <p className="text-[#9a8f7c] text-sm mb-7">Kostenlos. Kein Abo. Für immer.</p>

              {/* WG-Modus Toggle */}
              <div className="grid grid-cols-2 gap-2 mb-6 p-1 bg-[#1f1b16] rounded-xl">
                <button
                  type="button"
                  onClick={() => setWgMode('create')}
                  className={`flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
                    wgMode === 'create'
                      ? 'bg-[#3d5a3d] text-[#fdfbf6] shadow-sm'
                      : 'text-[#9a8f7c] hover:text-[#b8ac93]'
                  }`}
                >
              ...
              {wgMode === 'join' && (
                <div className="mb-5 bg-[#3d5a3d]/15 border border-[#3d5a3d]/30 rounded-xl px-4 py-3 text-sm text-[#8aa885]">
                  Nach der Anmeldung kannst du deinen Invite-Link eingeben.
                </div>
              )}

              <form onSubmit={handleMagicLink} className="space-y-4">
                <div>
                  <label htmlFor="name" className="block text-[11px] font-semibold text-[#9a8f7c] uppercase tracking-wider mb-2">
                    Dein Name
                  </label>
                  <div className="relative">
                    <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#5c5447] pointer-events-none" />
                    <input
                      id="name"
                      type="text"
                      value={name}
                ...
                <div>
                  <label htmlFor="email" className="block text-[11px] font-semibold text-[#9a8f7c] uppercase tracking-wider mb-2">
                    E-Mail
                  </label>
                  <div className="relative">
              ...
              <div className="flex items-center gap-3 my-5">
                ...
                <span className="text-[10px] text-[#5c5447] uppercase tracking-wider">oder</span>
                <div className="flex-1 h-px bg-[rgba(247,243,236,0.06)]" />
              </div>
              ...
              <p className="text-xs text-[#5c5447] mt-5 text-center">
                Mit der Registrierung stimmst du unserer{' '}
                <Link href="/datenschutz" className="text-[#8aa885] hover:text-[#c4694a] transition-colors">
                  Datenschutzerklärung
                </Link>{' '}
                zu.
              </p>

              <p className="text-sm text-[#5c5447] mt-3 text-center">
                Schon dabei?{' '}
                <Link href="/login" className="text-[#8aa885] hover:text-[#c4694a] font-medium transition-colors">
                  Anmelden →
                </Link>
              </p>
            </>
